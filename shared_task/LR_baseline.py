...
parser.add_argument("-st", "--sub_task", help="slot name of question for which we want to train the baseline", type=str,
                    required=True)
parser.add_argument("-o", "--output_dir", help="Path to the output directory where we will save all the model results",
...
def create_ngram_features_from(data):
    ...
    for text, chunk, tokenized_tweet_with_masked_chunk, gold_chunk, label in data:
        for n in Ns_for_NGRAMS:
...
def convert_data_to_feature_vector_and_labels(data, feature2i):
    ...
    for row_id, (text, chunk, tokenized_tweet_with_masked_chunk, gold_chunk, label) in enumerate(data):
        labels.append(label)
        for n in Ns_for_NGRAMS:
...
def main():
    ...
    # evaluation script
    F1, P, R, TP, FP, FN = get_TP_FP_FN(test_data, test_Y_prediction_probs, THRESHOLD=0.5)
